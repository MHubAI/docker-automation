...
BUILD_MODEL_CONF="../build/config/dev.yml"

TEST_CT_CHEST_CONF="../test/config/dev.yml"

RUN_ID=$(cat /dev/urandom | tr -cd 'a-f0-9' | head -c 32)
...
python ../build/run.py --config ${BUILD_MODEL_CONF} --ncores 1
...
python ../push/run.py --path_to_logs_folder ${TEST_LOG_DIR} --ncores 1 